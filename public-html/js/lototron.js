  ...
  */
  ...
  function testLototron(){
    ...
    //-- test method #getNextNumber()
      log("Test Lototron.getNextNumber() - returned data type");
        var maxNumber = 9;
        _loto = new Lototron( maxNumber );
        var temp = "";
        for (var indx=0; indx < maxNumber + 3; indx++){
          var res = _loto.getNextNumber();
          temp += typeof( res ) + ", ";
          if ( indx <= maxNumber ){
            if ( typeof( res ) != 'number' ){
              throw new Error("Returned invalid datatype from _loto.getNextNumber(). Result must be number.");
            }
          } else {
            if ( typeof( res ) != 'string' ){
              throw new Error("Returned invalid datatype from _loto.getNextNumber(). Result must be string.");
            }
          }
        }
      ...
      log("Test Lototron.getNextNumber() - random");
        var maxNumber = 9;
